...
// names converted to uppercase because the director BECAME
// DRUNK WITH POWER. Convert each first name into all caps
// and log the result
let allCaps = runners.map((arr) => {
...
// to an ordering issue.  Get a list of runners with
// large sized shirts so they can choose a different size.
// Return an array named largeShirts that contains information
// about the runners that have a shirt size of L and log
// the result
let largeShirts = runners.filter((arr) => {
...
let ticketPriceTotal = runners.reduce((total, arr) => {
	return total += arr.donation
}, 0)
console.log(`Total amount of donations is $${ticketPriceTotal}.`);

// ==== Challenge 5: Be Creative ====
// Now that you have used .forEach(), .map(), .filter(),
// and .reduce().  I want you to think of potential
// problems you could solve given the data set and the
// 5k fun run theme.  Try to create and then solve 3
// unique problems using one or many of the array methods
// listed above.

// Problem 1: How many shirts to order?
// How many of each size shirt needs to be ordered for the runners?

const smallShirts = runners.filter((arr) => {
	return arr.shirt_size == 'S'
})
const mediumShirts = runners.filter((arr) => {
	return arr.shirt_size == 'M'
})
const xlShirts = runners.filter((arr) => {
	return arr.shirt_size == 'XL'
})
const twoXlShirts = runners.filter((arr) => {
	return arr.shirt_size == '2XL'
})
const threeXlShirts = runners.filter((arr) => {
	return arr.shirt_size == '3XL'
})
console.log(`We need to order: ${smallShirts.length} small shirts, ${mediumShirts.length} medium shirts, ${largeShirts.length} large shirts, ${xlShirts.length} XL shirts, ${twoXlShirts.length} 2XL shirts, and ${threeXlShirts.length} 3XL shirts.`)

// Problem 2: A mass email must be sent to every runner. Create an array with every runners email.

const emailList = runners.forEach((arr, index) => {
